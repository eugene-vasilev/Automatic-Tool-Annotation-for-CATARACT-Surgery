...
def huffman_encode(arr, prefix, save_dir='./'):
    """
    Encode numpy array 'arr' and saves to `save_dir`
    ...
    datasize = dump(data_encoding, '{}/{}.bin'.format(directory, prefix))
    # Dump codebook (huffman tree)
    codebook_encoding = encode_huffman_tree(root, dtype)
    treesize = dump(codebook_encoding, '{}/{}_codebook.bin'.format(directory, prefix))
...
def huffman_decode(directory, prefix, dtype):
    ...
    # Read the codebook
    codebook_encoding = load('{}/{}_codebook.bin'.format(directory, prefix))
    root = decode_huffman_tree(codebook_encoding, dtype)

    # Read the data
    data_encoding = load('{}/{}.bin'.format(directory, prefix))
...
def huffman_encode_model(model, directory='encodings/'):
    ...
    for layer in model.layers:
        ...
        for input_num, weights in enumerate(weights_array):
            ...
            # Encode
            t0, d0 = huffman_encode(mat.data, '{}_{}_{}_data'.format(name, form, input_num), directory)
            t1, d1 = huffman_encode(mat.indices, '{}_{}_{}_indices'.format(name, form, input_num), directory)
            t2, d2 = huffman_encode(mat.indptr, '{}_{}_{}_indptr'.format(name, form, input_num), directory)
            print(mat.data.shape, mat.indices.shape, mat.indptr.shape)

            # Print statistics
            original = mat.data.nbytes + mat.indices.nbytes + mat.indptr.nbytes
            compressed = t0 + t1 + t2 + d0 + d1 + d2
            print('{} | {} {} x{} {}%'
                  .format(name, original, compressed, original / compressed, 100 * compressed / original))
        original_total += original
        compressed_total += compressed

    print('-' * 70)
    print('Total: | {} | {} | {}x | {}%'.format(original_total, compressed_total,
                                                original_total / compressed_total,
                                                100 * compressed_total / original_total))
...
def huffman_decode_model(model, directory='encodings/'):
    for num, layer in enumerate(model.layers):
        ...
        for input_num, weights in enumerate(weights_array):
            ...
            data = huffman_decode(directory, '{}_{}_{}_data'.format(name, form, input_num), dtype='float32')
            indices = huffman_decode(directory, '{}_{}_{}_indices'.format(name, form, input_num), dtype='int32')
            indptr = huffman_decode(directory, '{}_{}_{}_indptr'.format(name, form, input_num), dtype='int32')
            if not len(indices):
                indices = np.zeros_like(data)
            print(data.shape, indices.shape, indptr.shape)
            print(len(data), len(indices), len(indptr))
            mat = matrix((data, indices, indptr), weights.shape)
